...
const submit = () => {
    form.post(route("djamo.store"));
};
const update = () => {
    form.put(route("djamo.update", props.phone?.id));
};
const deleteForm = (id) => {
    form.delete(route("djamo.destroy", id));
};
</script>

<template>
    <AppLayout title="Configurer mon compte djamo">
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Configuration De Compte Djamo 
            </h2>
        </template>
        <div class="py-4">
            <div class="max-w-9xl mx-auto sm:px-6 lg:px-14">
                <div class="flex justify-center w-full">
                    <img
                        :src="url"
                        alt="Le logo de Djamo"
                        class="w-48 h-40 sm:w-96 sm:h-52 lg:w-96 lg:h-72"
                    />
                </div>
                <h3
